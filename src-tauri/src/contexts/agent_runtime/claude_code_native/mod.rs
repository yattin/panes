...
use claude_code_rs::api::{
    ApiClient, ModelStreamEvent, ToolCall as ClaudeToolCall, ToolCallFunction, ToolDefinition,
};
...
fn cuelight_action_display(
    tool_name: &str,
    args: &serde_json::Value,
    context: Option<&CueLightThreadContext>,
) -> Option<ActionDisplay> {
    if !tool_name.starts_with("cuelight_") {
        return None;
    }

    let project = context
        .map(|ctx| ctx.project_name.as_str())
    ...
    let label = match tool_name {
        ...
        "cuelight_get_story_bible" => "读取剧本圣经",
        "cuelight_update_story_bible" => "更新剧本圣经",
        "cuelight_get_visual_bible" => "读取视觉圣经",
        "cuelight_update_visual_bible" => "更新视觉圣经",
        "cuelight_list_characters" => "列出角色",
        "cuelight_get_character" => "读取角色",
        "cuelight_create_character" => "创建角色",
        "cuelight_update_character" => "更新角色",
        "cuelight_delete_character" => "删除角色",
        "cuelight_list_scenes" => "列出场景",
        "cuelight_get_scene" => "读取场景",
        "cuelight_create_scene" => "创建场景",
        "cuelight_update_scene" => "更新场景",
        "cuelight_delete_scene" => "删除场景",
        "cuelight_list_props" => "列出道具",
        "cuelight_get_prop" => "读取道具",
        "cuelight_create_prop" => "创建道具",
        "cuelight_update_prop" => "更新道具",
        "cuelight_delete_prop" => "删除道具",
        "cuelight_list_episodes" => "列出剧集",
        "cuelight_get_episode" => "读取剧集",
        "cuelight_create_episode" => "创建剧集",
        "cuelight_update_episode" => "更新剧集",
        "cuelight_delete_episode" => "删除剧集",
        "cuelight_list_storyboards" => "列出分镜",
        "cuelight_get_storyboard" => "读取分镜",
        "cuelight_create_storyboard" => "创建分镜",
        "cuelight_update_storyboard" => "更新分镜",
        "cuelight_delete_storyboard" => "删除分镜",
        "cuelight_batch_update_storyboards" => "批量更新分镜",
    ...
    let subtitle = match tool_name {
        "cuelight_download_original_script" => string_at(args, &["source_document_id"])
            .map(|id| format!("文档 ID {}", id))
            .unwrap_or_else(|| "最新原文文档".to_string()),
        ...
        | "cuelight_list_props"
        | "cuelight_list_episodes" => project.clone(),
        "cuelight_update_story_bible" | "cuelight_update_visual_bible" => {
            changed_fields_summary(args).unwrap_or_else(|| project.clone())
        }
        "cuelight_get_character" | "cuelight_update_character" | "cuelight_delete_character" => {
            first_display_value(
                args,
                &[
                    &["fields", "name"],
        ...
        "cuelight_create_character" => first_display_value(args, &[&["fields", "name"], &["name"]])
            .unwrap_or_else(|| project.clone()),
        "cuelight_get_scene" | "cuelight_update_scene" | "cuelight_delete_scene" => {
            first_display_value(
                args,
                &[&["fields", "name"], &["name"], &["scene_id"], &["sceneId"]],
            )
            .unwrap_or_else(|| project.clone())
        }
        "cuelight_create_scene" => first_display_value(args, &[&["fields", "name"], &["name"]])
            .unwrap_or_else(|| project.clone()),
        "cuelight_get_prop" | "cuelight_update_prop" | "cuelight_delete_prop" => {
            first_display_value(
                args,
                &[&["fields", "name"], &["name"], &["prop_id"], &["propId"]],
            )
            .unwrap_or_else(|| project.clone())
        }
        "cuelight_create_prop" => first_display_value(args, &[&["fields", "name"], &["name"]])
            .unwrap_or_else(|| project.clone()),
        "cuelight_get_episode" | "cuelight_update_episode" | "cuelight_delete_episode" => {
            first_display_value(
                args,
                &[
                    &["fields", "title"],
        ...
        "cuelight_create_episode" => first_display_value(
            args,
            &[
                &["fields", "title"],
                &["title"],
        ...
        .unwrap_or_else(|| project.clone()),
        "cuelight_list_storyboards" => first_display_value(
            args,
            &[&["episode_id"], &["episodeId"], &["fields", "episodeId"]],
        )
        .unwrap_or_else(|| project.clone()),
        "cuelight_get_storyboard" | "cuelight_update_storyboard" | "cuelight_delete_storyboard" => {
            first_display_value(
                args,
                &[
                    &["fields", "sceneNumber"],
        ...
        "cuelight_create_storyboard" => first_display_value(
            args,
            &[
                &["fields", "sceneNumber"],
                &["sceneNumber"],
        ...
        .unwrap_or_else(|| project.clone()),
        "cuelight_batch_update_storyboards" => storyboard_batch_summary(args)
            .or_else(|| first_display_value(args, &[&["episode_id"], &["episodeId"]]))
            .unwrap_or_else(|| project.clone()),
        "cuelight_upload_file" => file_name_arg(args, &["file_path"])
            .or_else(|| file_name_arg(args, &["path"]))
            .unwrap_or_else(|| project.clone()),
        "cuelight_generate_image" => first_display_value(
            args,
            &[
                &["prompt"],
                &["model"],
        ...
        .unwrap_or_else(|| project.clone()),
        "cuelight_generate_video" => first_display_value(
            args,
            &[
                &["prompt"],
                &["model"],
        ...
        .unwrap_or_else(|| project.clone()),
        "cuelight_task_status" => first_display_value(args, &[&["task_id"], &["taskId"]])
            .unwrap_or_else(|| project.clone()),
        "cuelight_list_models" => match string_at(args, &["media_type"]).as_deref() {
            Some("image") => "图片模型".to_string(),
            Some("video") => "视频模型".to_string(),
            _ => "全部模型".to_string(),
        },
        _ => project,
    };
    ...
    Some(ActionDisplay {
        ...
        subtitle: truncate_display_text(&subtitle, 48),
    })
}

fn first_display_value(args: &serde_json::Value, paths: &[&[&str]]) -> Option<String> {
    paths
        .iter()
        .find_map(|path| value_at(args, path).and_then(display_value))
}

fn display_value(value: &serde_json::Value) -> Option<String> {
    match value {
        serde_json::Value::String(s) => clean_display_text(s),
...
fn string_at(args: &serde_json::Value, path: &[&str]) -> Option<String> {
    value_at(args, path)
...
fn changed_fields_summary(args: &serde_json::Value) -> Option<String> {
    let fields = args.get("fields")?.as_object()?;
    let mut names = fields.keys().take(3).cloned().collect::<Vec<_>>();
    if names.is_empty() {
        return None;
    }
    names.sort();
    let suffix = if fields.len() > names.len() {
        " 等"
    } else {
        ""
    };
    Some(format!("更新 {}{}", names.join("、"), suffix))
}

fn storyboard_batch_summary(args: &serde_json::Value) -> Option<String> {
    let count = args
        .get("storyboards")
        .or_else(|| args.get("updates"))
        .and_then(|value| value.as_array())
        .map(|items| items.len())?;
    Some(format!("{count} 条分镜"))
}

fn file_name_arg(args: &serde_json::Value, path: &[&str]) -> Option<String> {
    string_at(args, path).map(|raw| {
...
impl Engine for ClaudeCodeNativeEngine {
    ...
    async fn send_message(
        ...
        for _round in 0..MAX_AGENT_ROUNDS {
            ...
            // 按协议无关的事件流消费：累积文本 delta（即时 emit）与工具调用分片（按 index 聚合）。
                    ...
                    event = stream.next() => match event {
                        ...
                        Some(Ok(event)) => match event {
                            ...
                            ModelStreamEvent::ToolCallDelta {
                                ...
                                if let Some(id) = id {
                                    tool_acc[index].id = Some(id);
                                }
                                if let Some(name) = name {
                                    tool_acc[index].name = Some(name);
                                }
                                if !arguments_delta.is_empty() {
                                    tool_acc[index].arguments.push_str(&arguments_delta);
                                }
                            }
            ...
            for call in &assistant_tool_calls {
                ...
                let action_display =
                    cuelight_action_display(&call.function.name, &args, cuelight_context.as_ref());
                ...
                let _ = event_tx
                    .send(EngineEvent::ActionStarted {
                        ...
                        summary,
                        display_label: action_display.as_ref().map(|display| display.label.clone()),
                        display_subtitle: action_display
                            .as_ref()
                            .map(|display| display.subtitle.clone()),
                        details: args.clone(),
                    })
                    .await;
...
mod tests {
    ...
    use crate::engines::cuelight_tools::{build_cuelight_tool_definitions, set_global_auth_token};
    ...
    fn cuelight_test_context() -> CueLightThreadContext {
        CueLightThreadContext {
            project_id: "project-1".to_string(),
            project_name: "林渊在原著的升级路线".to_string(),
            project_type: Some("drama".to_string()),
    ...
    #[test]
    fn cuelight_display_covers_all_exposed_tools() {
        let context = cuelight_test_context();
        ...
        assert!(!names.is_empty(), "expected CueLight tool definitions");
        for name in names {
            let display = cuelight_action_display(name, &json!({}), Some(&context))
                .unwrap_or_else(|| panic!("missing display metadata for {name}"));
            assert!(!display.label.trim().is_empty(), "empty label for {name}");
            assert_ne!(display.label, name, "label should not be raw function name");
            assert!(
                !display.subtitle.trim().is_empty(),
                "empty subtitle for {name}"
            );
        }
    }

    #[test]
    fn cuelight_display_extracts_semantic_subtitles() {
        let context = cuelight_test_context();
        let cases = [
            (
            ...
            (
                ...
                json!({ "fields": { "name": "地下仓库" } }),
                "创建场景",
                "地下仓库",
            ),
            (
                "cuelight_create_prop",
                json!({ "fields": { "name": "褪色纸条" } }),
                "创建道具",
                "褪色纸条",
            ),
            (
                "cuelight_create_episode",
                json!({ "fields": { "title": "第一集：热浪前夜" } }),
                "创建剧集",
                "第一集：热浪前夜",
            ),
            (
            ...
            (
                ...
                json!({ "file_path": r"C:\cue-work\proj2\refs\hero.png" }),
                "上传参考文件",
                "hero.png",
            ),
            (
        ...
        for (tool, args, expected_label, expected_subtitle) in cases {
            let display = cuelight_action_display(tool, &args, Some(&context)).unwrap();
            assert_eq!(display.label, expected_label);
            assert_eq!(display.subtitle, expected_subtitle);
        }
    }

    #[test]
    fn cuelight_display_falls_back_to_project_and_truncates_text() {
        let context = cuelight_test_context();
        let display =
            cuelight_action_display("cuelight_project_status", &json!({}), Some(&context)).unwrap();
        assert_eq!(display.subtitle, "林渊在原著的升级路线");

        let long_prompt = "第一场  田雨在极热城市中发现丈夫留下的秘密文件并决定反击，随后进入地下仓库寻找补给，并在断电前联系父母安排撤离路线";
        let display = cuelight_action_display(
            "cuelight_generate_image",
            &json!({ "prompt": long_prompt }),
            Some(&context),
        )
        .unwrap();
        assert!(display.subtitle.ends_with('…'));
        assert!(display.subtitle.chars().count() <= 49);
        assert!(!display.subtitle.contains("  "));

        let display = cuelight_action_display("cuelight_list_models", &json!({}), None).unwrap();
        assert_eq!(display.subtitle, "全部模型");
    }
    ...
    /// 真实流式测试：直接通过 `ApiClient::chat_stream()` 按统一模型事件增量消费，
    ...
    async fn real_chat_stream() {
        ...
        while let Some(event_result) = stream.next().await {
            let event = event_result.expect("stream event should be readable");
        ...
        assert!(
            ...
            "expected at least one content chunk from the model stream"
        );
        assert!(
            !assembled.trim().is_empty(),
            "expected non-empty assembled stream content, got {} chunks",
            content_chunks.len()
        );
        for n in 1..=5 {
            assert!(
