//! File Edit Tool

use super::{Tool, ToolError, ToolOutput};
use async_trait::async_trait;
use serde_json;
use std::collections::HashMap;
use std::path::Path;

const MAX_EDIT_FILE_SIZE: u64 = 1024 * 1024 * 1024;

pub struct FileEditTool;

impl Default for FileEditTool {
    fn default() -> Self {
        Self::new()
    }
}

impl FileEditTool {
    pub fn new() -> Self {
        Self
    }
}

#[async_trait]
impl Tool for FileEditTool {
    fn name(&self) -> &str {
        "file_edit"
    }

    fn description(&self) -> &str {
        "Edit a file by replacing specific content"
    }

    fn input_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "Path to the file to edit"
                },
                "old_content": {
                    "type": "string",
                    "description": "Content to find and replace"
                },
                "new_content": {
                    "type": "string",
                    "description": "New content to replace with"
                },
                "old_string": {
                    "type": "string",
                    "description": "Alias for old_content"
                },
                "new_string": {
                    "type": "string",
                    "description": "Alias for new_content"
                },
                "replace_all": {
                    "type": "boolean",
                    "description": "Replace all matches instead of requiring exactly one match"
                }
            },
            "required": ["file_path"]
        })
    }

    fn tool_summary(&self, input: &serde_json::Value) -> Option<String> {
        input["file_path"]
            .as_str()
            .map(|path| format!("edit {}", path))
    }

    async fn execute(&self, input: serde_json::Value) -> Result<ToolOutput, ToolError> {
        let file_path = input["file_path"].as_str().ok_or_else(|| ToolError {
            message: "file_path is required".to_string(),
            code: Some("missing_parameter".to_string()),
        })?;

        let old_content = input["old_content"]
            .as_str()
            .or_else(|| input["old_string"].as_str())
            .ok_or_else(|| ToolError {
                message: "old_content or old_string is required".to_string(),
                code: Some("missing_parameter".to_string()),
            })?;

        let new_content = input["new_content"]
            .as_str()
            .or_else(|| input["new_string"].as_str())
            .ok_or_else(|| ToolError {
                message: "new_content or new_string is required".to_string(),
                code: Some("missing_parameter".to_string()),
            })?;

        if old_content == new_content {
            return Err(ToolError {
                message: "No changes to make: old and new content are identical".to_string(),
                code: Some("no_change".to_string()),
            });
        }

        if old_content.is_empty() {
            return Err(ToolError {
                message: "old content cannot be empty".to_string(),
                code: Some("empty_old_content".to_string()),
            });
        }

        let path = Path::new(file_path);

        if !path.exists() {
            return Err(ToolError {
                message: format!("File does not exist: {}", file_path),
                code: Some("file_not_found".to_string()),
            });
        }

        let metadata = std::fs::metadata(path).map_err(|e| ToolError {
            message: format!("Failed to inspect file: {}", e),
            code: Some("metadata_error".to_string()),
        })?;

        if metadata.len() > MAX_EDIT_FILE_SIZE {
            return Err(ToolError {
                message: format!("File is too large to edit ({} bytes)", metadata.len()),
                code: Some("file_too_large".to_string()),
            });
        }

        let content = std::fs::read_to_string(path).map_err(|e| ToolError {
            message: format!("Failed to read file: {}", e),
            code: Some("read_error".to_string()),
        })?;

        let match_count = content.matches(old_content).count();
        if match_count == 0 {
            return Err(ToolError {
                message: "old content not found in file".to_string(),
                code: Some("content_not_found".to_string()),
            });
        }

        let replace_all = input["replace_all"].as_bool().unwrap_or(false);
        if match_count > 1 && !replace_all {
            return Err(ToolError {
                message: format!(
                    "old content appears {} times. Provide replace_all=true or make old content more specific.",
                    match_count
                ),
                code: Some("multiple_matches".to_string()),
            });
        }

        let new_file_content = if replace_all {
            content.replace(old_content, new_content)
        } else {
            content.replacen(old_content, new_content, 1)
        };

        std::fs::write(path, new_file_content).map_err(|e| ToolError {
            message: format!("Failed to write file: {}", e),
            code: Some("write_error".to_string()),
        })?;

        let mut result_metadata = HashMap::new();
        result_metadata.insert("file_path".to_string(), serde_json::json!(file_path));
        result_metadata.insert(
            "matches_replaced".to_string(),
            serde_json::json!(if replace_all { match_count } else { 1 }),
        );
        result_metadata.insert("replace_all".to_string(), serde_json::json!(replace_all));

        Ok(ToolOutput {
            output_type: "text".to_string(),
            content: format!(
                "Successfully edited {} ({} replacement{})",
                file_path,
                if replace_all { match_count } else { 1 },
                if (replace_all && match_count != 1) || (!replace_all && match_count > 1) {
                    "s"
                } else {
                    ""
                }
            ),
            metadata: result_metadata,
        })
    }
}
