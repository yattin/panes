...
## 更新摘要
**所做更改**
- 新增 conversation application 模块的全面文档
- 添加聊天应用、审批、附件、输入处理、消息查询、策略、审查、转块、通知和流式传输功能详解
- 更新架构图以反映新的 conversation application 层
- 扩展聊天消息管理和状态机章节
- 新增附件处理和输入验证机制说明
- 增强流式传输和事件处理流程描述
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [Conversation Application 模块详解](#conversation-application-模块详解)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
...
## 引言
本文件系统性梳理聊天与 AI 集成模块的设计与实现，特别关注新增的 conversation application 模块。该模块提供了完整的聊天应用框架，包括聊天应用、审批、附件、输入处理、消息查询、策略、审查、转块、通知和流式传输等功能。覆盖以下主题：
- AI 引擎抽象层设计：统一接口、能力边界、生命周期与健康检查
- Conversation Application 模块：聊天应用层的完整实现
...
## 项目结构
前端采用 React + Zustand 状态管理，后端以 Tauri Rust 引擎子系统为核心，通过 IPC 桥接前后端。新增的 conversation application 模块位于 `src-tauri/src/contexts/conversation/application/chat/` 目录下，提供完整的聊天应用实现。
...
## 架构总览
整体采用"前端 UI + 状态管理 + IPC + Conversation Application 模块 + 后端引擎"的分层架构。Conversation Application 模块作为中间层，协调前端与后端引擎的交互，提供完整的聊天应用功能。
...
Eng-->>EngMgr : 产生事件(TextDelta/Action/Approval/Usage/Notice/Error)
