...
- [terminal_notifications.rs:1-60](file://src-tauri/src/terminal_notifications.rs#L1-L60)
...
- [mod.rs:44-90](file://src-tauri/src/terminal/mod.rs#L44-L90)
- [osc_notifications.rs:10-44](file://src-tauri/src/terminal/osc_notifications.rs#L10-L44)
...
## 架构总览
终端系统采用“前端 xterm + 后端 PTY”双层架构，通过 IPC 事件进行解耦。后端负责 PTY 生命周期与输出节流，前端负责渲染与交互体验；通知系统贯穿前后端，支持 OSC 协议与 TCP 入口两种来源。
...
UI->>Store : 创建/切换/分割会话
Store->>IPC : 调用 terminalCreateSession(...)
IPC->>TM : invoke("terminal_create_session", ...)
...
### PTY 系统集成与输出节流（mod.rs）
- PTY 生命周期
  - spawn_session 创建会话，记录会话元数据与 shell PID；spawn_reader 启动后台线程读取输出。
- 输出缓冲与节流
  - 后台共享缓冲区，发射线程以约 60Hz 聚合输出，限制单次最大字节数，避免 UI 卡顿。
- 回放缓冲
  - 记录最近输出片段，支持从指定序号恢复，上限由常量控制，避免内存膨胀。
- 前台进程检测
  - 定期探测 shell PID 对应前台进程，推送“前台变更”事件，便于 UI 展示当前活跃进程。
- 环境注入
  - 注入 TERM/COLORTERM/PATH 等环境变量，Windows 下补齐用户目录与缓存路径，确保兼容性。

```mermaid
sequenceDiagram
participant Reader as "Reader线程"
participant Buffer as "共享缓冲"
participant Emitter as "发射线程"
participant IPC as "IPC事件"
participant UI as "前端"
Reader->>Buffer : push_chunk(UTF-8片段)
Emitter->>Buffer : take_output_chunks_head(限流)
Emitter->>IPC : emit terminal-output-事件
IPC-->>UI : 监听回调
UI->>UI : 解码/拼接/刷新
```
...
- [mod.rs:760-820](file://src-tauri/src/terminal/mod.rs#L760-L820)
...
## 结论
该终端系统通过“前端 xterm + 后端 PTY”的清晰分层，实现了高性能、可诊断、可扩展的终端体验。前端负责渲染与交互，后端专注 PTY 生命周期与输出节流，通知系统打通 OSC 与 TCP 两条通道，配合会话缓存与工作树联动，满足多会话、多工作区的复杂使用场景。建议在高吞吐场景下启用加速渲染并在出现异常时利用诊断导出来定位问题。
